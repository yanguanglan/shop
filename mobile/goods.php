...
if (!$smarty->is_cached('goods.dwt', $cache_id))
{
    ...
    else
    {
        ...
        */
        ...
        $smarty->assign('sales_count',          get_goods_sales_count($goods_id)); // by  Leah $sales_count 销售量
        //获取tag
...
function get_also_bought($goods_id)
{
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        $comment = assign_comment($row['goods_id'] ,0 ) ;   // by Leah
        $arr[$key]['goods_comment']   = $comment['comments']; // by Leah  购买改商品的会员还买了的商品评论
        if ($row['promote_price'] > 0)
        {
