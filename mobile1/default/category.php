...
$sort  = (isset($_REQUEST['sort'])  && in_array(trim(strtolower($_REQUEST['sort'])), array('goods_id', 'shop_price', 'last_update' , 'click_count'))) ? trim($_REQUEST['sort'])  : $default_sort_order_type;//by Leah
...
if (!$smarty->is_cached('category.dwt', $cache_id))
{
    ...
    if ($cat['filter_attr'] > 0)
    {
        ...
        foreach($cat as $key => $val)
        {
            if(is_array($val))
            {
                $cat_filter_attr_arr[$key] = $val['filter_attr'];
                if($length == $key)
                {
                    $cat_filter_attr_arr_res = array_unique($cat_filter_attr_arr);
                    foreach($cat_filter_attr_arr_res as $val_attr)
                    {
                        $cat_filter_attr_str .= $val_attr  . ',';
                    }
                    $cat_filter_attr_str = substr($cat_filter_attr_str, 0, -1);
                    $cat_filter_attr = explode(',', $cat_filter_attr_str);
                }
            }
            else
            {
        ...
        foreach ($cat_filter_attr AS $key => $value)
        {
            ...
            if($temp_name = $db->getOne($sql))
            {
                ...
                $sql = "SELECT a.attr_id, g.cat_id, MIN(a.goods_attr_id ) AS goods_id, a.attr_value AS attr_value FROM " . $ecs->table('goods_attr') . " AS a, " . $ecs->table('goods') .
                       " AS g" .
                       " WHERE ($children " . ') AND g.goods_id = a.goods_id AND g.is_delete = 0 AND g.is_on_sale = 1 AND g.is_alone_sale = 1 '.
                       " AND a.attr_id='$value' ".
				...
				$all_attr_list[$key]['attr_list'][0]['goods_id'] = 0;
                ...
                foreach ($attr_list as $k => $v)
                {
                   ...
                   // $temp_arrt_url_arr[$key] = $v['goods_id'];       //为url中代表当前筛选属性的位置变量赋值,并生成以'.'分隔的筛选属性字符串
                    if($filter_attr_str != "" && (!empty($filter_attr[$key])))
                    {
                        $temp_arrt_url_arr[$key] = $filter_attr[$key] . '_' . $v['goods_id'];
                    }
                    else
                    {
					...
					$temp_arrt_url = implode('.', $temp_arrt_url_arr);
					$split_attr_arr = explode('_', $filter_attr[$key]);
                    
                    if (!empty($filter_attr[$key]) && in_array($v['goods_id'], $split_attr_arr))
                    {       
					...
					$all_attr_list[$key]['attr_list'][$temp_key]['attr_id'] = $v['attr_id'];
					...
					if($all_attr_list[$key]['attr_list'][$temp_key]['selected'] == 1)
                    {
                        $attr_str = '';
                        $attr_arr = array();
                        if(!(preg_match('/_/', $filter_attr[$key])))
                        {
                            $attr_str = str_replace($v['goods_id'], 0, $filter_attr_str);
                        }
                        else
                        {
                            $attr_str = str_replace('_' . $v['goods_id'], '', $filter_attr_str);
                            $attr_str = str_replace($v['goods_id'] . '_', '', $attr_str);
                            $attr_str = str_replace($v['goods_id'], '', $attr_str);
                        }
//                        
                        $all_attr_list[$key]['attr_list'][$temp_key]['del_url'] = build_uri('category', array('cid'=>$cat_id, 'bid'=>$brand, 'price_min'=>$price_min, 'price_max'=>$price_max, 'filter_attr'=>$attr_str), $cat['cat_name']);
                    }
                }
            }
        ...
        if (!empty($filter_attr))
        {
            ...
            {
                if (is_numeric($v) && $v !=0 && isset($cat_filter_attr[$k]))  //判断属性id是否为空和属性值是否存在。
                {
                ...
                {
                    //处理1_2_3这种格式的字符串     @author  guanchengle  update 2013-09-18
                    if($v != 0 && isset($cat_filter_attr[$k]))
                    {
                        $attr_value_arr = explode('_', $v);
                        foreach($attr_value_arr as $attr_value_arr_val)
                        {
                            $attr_value_arr_val_str .= $attr_value_arr_val . ',';
                        }
                        $attr_value_arr_val_str = substr($attr_value_arr_val_str, 0, -1);
                        $sql = $ext_sql . "b.attr_value = a.attr_value AND b.attr_id = " . $cat_filter_attr[$k] ." AND a.goods_attr_id IN (" . $attr_value_arr_val_str . ')';
                        $ext_group_goods = $db->getColCached($sql);
                        $ext .= ' AND ' . db_create_in($ext_group_goods, 'g.goods_id');
                    }
                }
    ...
    /*分类筛选*/
            ...
            $categorys = get_cat_child($cat_id);
            
            
            foreach ($categorys AS $key => $val)
            {
                $temp_key = $key + 1;
                $categorys[$temp_key]['cat_name'] = $val['cat_name'];
                $categorys[$temp_key]['parent_id']= $val['parent_id'];
                
                $categorys[$temp_key]['url'] = build_uri('category', array('cid' => $val['cat_id']), $cat['cat_name']);
                $categorys[$temp_key]['del_url'] = build_uri('category', array('cid' => $val['parent_id']), $cat['cat_name']);
//                 $categorys[$temp_key]['url'] = build_uri('category', array('cid' => $val['cat_id']), $cat['cat_name']);
                /* 判断分类是否被选中 */
                if ($cat_id == $categorys[$key]['cat_id'] || in_array($categorys[$key]['cat_id'], $cat_id))
                {
                    $categorys[$temp_key]['selected'] = 1;
                }
                else
                {
                    $categorys[$temp_key]['selected'] = 0;
                }
            }
            
            $categorys[0]['cat_name'] = $_LANG['all_attribute'];
            $categorys[0]['url'] = build_uri('category', array('cid' => $categorys[0]['parent_id'], 'bid'=>$brand), $cat['cat_name']);
            if($cat_id == $categorys[0]['parent_id'])
            {
                $categorys[0]['selected'] = 1;
            }
            else
            {
                 $categorys[0]['selected'] = 0;
            }
    ...
    $choose_res = array(
        0               =>  get_choose_category($categorys),
        1               =>   get_choose_category($price_grade),
        2               =>  array(
                            'attr_list'  => get_choose_attr($all_attr_list, $filter_attr_str),
        ),
    );

    if((!empty($choose_res[1])) || (!empty($choose_res[2]['attr_list'])))
    {
            $smarty->assign('gt', "&gt;");
    }

    $smarty->assign('choose_res',$choose_res);
...
function get_cat_child($cat_id){
	...
	if($res['parent_id'] == 0) {
		//如果父ID等于0则查出所有的子分类
		$sql = "select cat_id,cat_name,parent_id from " . $GLOBALS['ecs']->table('category') . " where parent_id = '$cat_id'";
		$res = $GLOBALS['db']->getAll($sql);
		return $res;
	}
	else
	{
		$result = get_cat_child($res['parent_id']);
		if(!empty($result))
		{
			return $result;
		}
 ...
 * @author guanchengle
 */
function get_choose_attr($attr=array(), $filter_attr_value)
{
    static $url = '';
    $attr_arr = array();
    $a_key = 0;
    ...
    {
        foreach($value['attr_list'] as $k_attr => $v_attr)
        {
            if($v_attr['goods_id'] === 0)
            {
                $url = $v_attr['url'];
            }
            if($v_attr['selected'] == 1 && (!in_array('全部', $v_attr)) && $v_attr['goods_id'] != 0)
            {
                    $attr_arr[$a_key] = $v_attr;
                    $attr_arr[$a_key]['url'] = str_replace('_'.$v_attr['goods_id'], '', $v_attr['url']);
                    $attr_arr[$a_key]['url'] = str_replace($v_attr['goods_id'], '0', $attr_arr[$a_key]['url']);
                    $a_key++;
            }
        }
    }
...
function get_choose_category($result=array())
{
    static $url = '';
    $choose_value = array();
    foreach($result as $key => $value)
    {
        ...
        if($value['selected'] == 1 && (!in_array('全部', $value)))
        {
            $choose_value = $value;
            $choose_value['url'] = $url;
        }
    }
    return $choose_value;
}
