...
?>
       ...
       <?php if ($this->_var['action'] == 'pre_list'): ?>
       <h5><span>我的预定订单</span></h5>
       <div class="blank"></div>
       <table width="100%" border="0" cellpadding="5" cellspacing="1" bgcolor="#dddddd">
          <tr align="center">
            ...
            <td bgcolor="#ffffff">在线支付金额</td>
            <td bgcolor="#ffffff">实体店支付金额</td>
            <td bgcolor="#ffffff">订单状态</td>
            <td bgcolor="#ffffff">操作</td>
          </tr>
          <?php $_from = $this->_var['pres']; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array'); }; $this->push_vars('', 'item');if (count($_from)):
    foreach ($_from AS $this->_var['item']):
?>
          <tr>
            <td align="center" bgcolor="#ffffff">
              <a href="paypre.php?id=<?php echo $this->_var['item']['pre_id']; ?>" class="f6"><?php echo $this->_var['item']['pre_id']; ?></a>
            </td>
            <td align="center" bgcolor="#ffffff"><?php echo $this->_var['item']['created']; ?></td>
            <td align="left" bgcolor="#ffffff">
              <p><?php echo $this->_var['item']['goods_name']; ?></p>
              <?php if ($this->_var['item']['onlinepay'] == 1): ?>
              <p>入住时间：<?php echo $this->_var['item']['startdate']; ?></p>
              <p>离店时间：<?php echo $this->_var['item']['enddate']; ?></p>
              <?php else: ?>
              <p>消费时间：<?php echo $this->_var['item']['startdate']; ?></p>
              <?php endif; ?>
              <p>房间:<?php echo $this->_var['item']['rooms']; ?></p>
            </td>
            <td align="center" bgcolor="#ffffff"><?php echo $this->_var['item']['totalprice']; ?>元</td>
            <td align="center" bgcolor="#ffffff"><?php echo $this->_var['item']['onlineprice']; ?>元</td>
            <td align="center" bgcolor="#ffffff"><?php echo $this->_var['item']['shopprice']; ?>元</td>
            <td align="center" bgcolor="#ffffff"><?php if ($this->_var['item']['onlinepay'] == 1): ?><?php if ($this->_var['item']['status'] == 1): ?>待付款<?php elseif ($this->_var['item']['status'] == 2): ?>已付款<?php else: ?>已取消<?php endif; ?><?php else: ?><?php if ($this->_var['item']['status'] == 2): ?>已预订<?php else: ?>已取消<?php endif; ?><?php endif; ?></td>
            <td align="center" bgcolor="#ffffff"><font class="f6"><?php if ($this->_var['item']['status'] == 1): ?><?php if ($this->_var['item']['onlinepay'] == 1): ?><a href="paypre.php?id=<?php echo $this->_var['item']['pre_id']; ?>">我要支付</a>&nbsp;<?php endif; ?><a href="user.php?act=cancel_pre&pre_id=<?php echo $this->_var['item']['pre_id']; ?>" onclick="if(!confirm('确定取消吗?')){return false;}">取消订单</a><?php else: ?>无<?php endif; ?></font></td>
          </tr>
          <?php endforeach; endif; unset($_from); ?><?php $this->pop_vars();; ?>
          </table>
        <div class="blank5"></div>
