...
<div class="tab-div">
    ...
    <!-- tab body -->
    <div id="tabbody-div">
      <form id="theForm" enctype="multipart/form-data" action="" method="post" name="theForm" >
        <!-- 鏈€澶ф枃浠堕檺鍒 -->
...
<!--            <td><input type="text" name="goods_number" value="<?php echo $this->_var['goods']['goods_number']; ?>" size="20" <?php if ($this->_var['code'] != '' || $this->_var['goods']['_attribute'] != ''): ?>readonly="readonly"<?php endif; ?> /><br />-->
          ...
          <tr>
            <td class="label">是否在线支付</td>
            <td><input type="checkbox" name="onlinepay" <?php if ($this->_var['goods']['onlinepay'] == 1): ?>checked="true"<?php endif; ?> value=1 /></td>
          </tr>
          <tr>
            <td class="label"> 酒店房间价格 </td>
            <td>
              <div id="roomX">
                  <table>
                      <thead>
                      ...
                      <tbody>
                        <tr>
                         ...
                         <td><input type="text"></td>
                         <td><input type="text"></td>
                         <td><a class="J_roomDel cursor">删除</a></td>                         
                        </tr>
                      </tbody>
                  </table>
                  <a id="J_roomAdd" class="cursor">增加一行</a>
              </div>
              <textarea id="J_realRoom" name="rooms" style="display:none;"><?php echo $this->_var['goods']['rooms']; ?></textarea><br />不属于酒店预定类，请为空
            </td>
          </tr>
        </table>
        <style>
          ...
          <script>
             ...
             (function() {
                 $(function() {
                     var $tbody = $('#roomX tbody');
                     var _val = $('#J_realRoom').val();
                     $('#J_roomAdd').bind('click', function() {
                        var $td = $('<tr><td><input type="text"></td><td><input type="text"></td><td><input type="text"></td><td><a class="J_roomDel cursor">删除</a></td></tr>');
                        $td.appendTo($tbody);
                     });
                     $('.J_roomDel').live('click', function(e) {
                         $(this).parent().parent().remove();
                     });
                     if ($('#roomX').length) {
                        var initRoom = JSON.parse(_val);
                        var htmls = '';
                        var catroom = initRoom.catroom;
                         if (!!catroom) {
                          for(var key in catroom) {
                              if (key != 'toJSONString') {
                                var _key = key;
                                var _val = catroom[key].split('|');
                                htmls += '<tr><td><input type="text" value="'+_key+'"></td><td><input type="text" value="'+_val[0]+'"></td><td><input type="text" value="'+_val[1]+'"></td><td><a class="J_roomDel cursor">删除</a></td></tr>';                              
                              }
                          }
                          if (!!htmls) {
                             $tbody.html('');
                             $(htmls).appendTo($tbody);
                          }
                        }
...
?>
        ...
        <div class="button-div">
          ...
          <?php endif; ?>
          <input id="J_submit" type="button" value="<?php echo $this->_var['lang']['button_submit']; ?>" class="button" onclick="validate('<?php echo $this->_var['goods']['goods_id']; ?>')" />
...
?>
  ...
  {
      if ($('#roomX').length) {
         var trs = $('#roomX tbody tr');
         var rooms = {};
         $(trs).each(function(index, item) {
             var inputs = $(item).find('input');
             var id = $(inputs[0]).val();
             if (!!id){
               rooms[id] = ($(inputs[1]).val() || 0) + '|' + ($(inputs[2]).val() || 0);
             }
         });
         $('#J_realRoom').val(JSON.stringify({catroom:rooms}));
      }
