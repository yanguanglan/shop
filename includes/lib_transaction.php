...
function get_user_pres($user_id, $num = 10, $start = 0)
{
    /* 取得订单列表 */
    $arr    = array();
...
function cancel_pre($pre_id, $user_id = 0){
  /* 查询订单信息，检查状态 */
    $sql = "SELECT * FROM " .$GLOBALS['ecs']->table('pre_order') ." WHERE pre_id = '$pre_id'";
    $order = $GLOBALS['db']->GetRow($sql);
    if($order['user_id']!=$user_id){
        $GLOBALS['err']->add('只能取消自己的订单');
        return false;
    }
    if($order['status']!=1){
        $GLOBALS['err']->add('订单状态只有待付款才可以取消');
        return false;
    }
  $sql="UPDATE " .$GLOBALS['ecs']->table('pre_order') ." SET status=0 WHERE pre_id = '$pre_id'";
