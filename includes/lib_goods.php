...
function get_category_recommend_goods($type = '', $cats = '', $brand = 0, $min =0,  $max = 0, $ext='')
{
    ...
    $sql =  'SELECT g.goods_id, g.cat_id, c.parent_id, g.goods_name, g.goods_name_style, g.market_price, g.shop_price AS org_price, g.promote_price, ' .
            ...
            "LEFT JOIN " . $GLOBALS['ecs']->table('member_price') . " AS mp ".
                    "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' ".
            'WHERE g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 ' . $brand_where . $price_where . $ext;  //by Leah
    $num = 0;
 ...
 */
    if (!empty($cats))
    { 
      // $where = $cats > 0 ? get_children($cats) : '';
       $sql .= " AND ( ".$cats . " OR " . get_extension_goods($cats) . ")";
    }
    /**
     * by Leah E
...
{
    $time = gmtime();
    $sql = 'SELECT g.*, c.measure_unit, b.brand_id, b.brand_name AS goods_brand, m.type_money AS bonus_money,count( r.id_value) as commnet_count,' .
            ...
            "GROUP BY g.goods_id";  //by leah add count( r.id_value) as commnet_count,
